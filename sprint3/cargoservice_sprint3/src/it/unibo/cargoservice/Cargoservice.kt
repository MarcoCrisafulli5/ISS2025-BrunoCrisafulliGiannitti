...
class Cargoservice ( name: String, scope: CoroutineScope, isconfined: Boolean=false, isdynamic: Boolean=false ) : 
	...
	override fun getBody() : (ActorBasicFsm.() -> Unit){
		...
		//IF actor.withobj !== null val actor.withobj.name» = actor.withobj.method»ENDIF
		        ...
		        val _gui = run {
		            it.unibo.ctxcargoholdservice.CoapServerSetup.start()
		            it.unibo.ctxcargoholdservice.GuiWebServer.start()
		            it.unibo.ctxcargoholdservice.WsServer.start()
		        }
		...
		return { //this:ActionBasciFsm
				...
				state("requestrefused") { //this:State
					action { //it:State
						...
						CommUtils.outblack("---------------------------")
						if(  SLOT != 0  
						 ){forward("slotrelease", "slotrelease($SLOT)" ,"hold" ) 
						}
						delay(500) 
						//genTimer( actor, state )
					}
